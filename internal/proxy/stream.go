...
func (p *StreamProxy) buildJellyfinStreamURL(itemID, path, query string, session *models.ShareSession) string {
	...
	// are generated by Jellyfin and carry the parameters its running ffmpeg job was
	// started with; stripping those would make the request stop matching that job
	// and can force Jellyfin to tear the transcode down mid-playback.
	if session != nil && (session.AudioStreamIndex.Valid || session.SubtitleStreamIndex.Valid) {
...
func (p *StreamProxy) ServeSubtitle(w http.ResponseWriter, r *http.Request) {
	...
	// Same gate as ServeStream: a session that stopped sending heartbeats is over,
	// and its subtitle text should not stay readable either.
	if !session.IsActive(p.cfg.SessionHeartbeatTimeout) {
		http.Error(w, "session timed out", http.StatusForbidden)
		return
	}
	...
	if resp.StatusCode >= 200 && resp.StatusCode < 300 {
		...
		// private: tied to one session, must not sit in a shared cache. Cache only
		// success - a transient Jellyfin failure stored as text/vtt would leave the
		// viewer without subtitles for an hour after Jellyfin recovered.
