...
func (p *StreamProxy) buildJellyfinStreamURL(itemID, path, query string) string {
	...
	// No credential in the URL: Jellyfin 12 rejects api_key, and proxyRequest
	// authorizes by header instead.
...
func (p *StreamProxy) ServeImage(w http.ResponseWriter, r *http.Request) {
	...
	// Sizing params only; the credential travels as a header.
	params := url.Values{}
